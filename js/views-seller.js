  ...
  profile: function() {
    ...
    const html = `<div class="page-title"><div><h1>Hồ sơ của tôi</h1><p>Thông tin thành viên, tài khoản nhận hoa hồng, mật khẩu</p></div></div>
      <div class="grid-2">
        ...
        <div class="stack">
          <div class="card"><div class="card-head"><h2>Thông tin cá nhân</h2></div><div class="card-body"><form novalidate onsubmit="Seller.saveProfile(event)">${UI.field({ id: 'pf-name', label: 'Họ và tên', required: true, value: u.fullName })}${UI.field({ id: 'pf-phone', label: 'Số điện thoại (định danh, không đổi)', value: u.phone, mono: true, attrs: 'readonly' })}${UI.field({ id: 'pf-email', label: 'Email', type: 'email', required: true, value: u.email })}${UI.field({ id: 'pf-address', label: 'Địa chỉ', type: 'textarea', rows: 2, value: u.address })}<div class="actions"><button class="btn btn-primary btn-lg">Lưu thay đổi</button></div></form>
            <div class="section mt-6" style="border-top:1px solid var(--border-soft);padding-top:20px"><h3 class="mb-3">Đổi mật khẩu</h3><form novalidate onsubmit="Seller.changePassword(event)">${UI.field({ id: 'pw-old', label: 'Mật khẩu hiện tại', type: 'password', required: true })}<div class="form-grid form-grid-2 mt-4">${UI.field({ id: 'pw-new', label: 'Mật khẩu mới', type: 'password', required: true, hint: 'Tối thiểu 8 ký tự, có chữ và số' })}${UI.field({ id: 'pw-new2', label: 'Nhập lại mật khẩu mới', type: 'password', required: true })}</div><div class="actions"><button class="btn btn-outline btn-lg">Đổi mật khẩu</button></div></form></div>
          </div></div>
        </div>
      </div>`;
