 ...
 */
const Buyer = {
  /** Panel giới thiệu bên trái cho màn có form (A-04, A-05, A-06) — website-first. */
  ...
  A01: function(q) {
    ...
    if (state === 'invalid' || state === 'inactive' || (!seller && !['inactive-pkg'].includes(state))) {
      return App.buyerShell(TPL.render('a-01-state', { state: state === 'inactive' ? 'inactive' : 'invalid', seller, phone: '' }), { narrow: true });
    }
    // (f) Gói ngưng bán
    if (state === 'inactive-pkg' || !pkg) return App.buyerShell(TPL.render('a-01-state', { state: 'inactive-pkg', seller, phone: '' }), { showRef: true, narrow: true });
    // (d) SĐT đã mua gói (BR-03)
    if (state === 'duplicate') return App.buyerShell(TPL.render('a-01-state', { state: 'duplicate', seller, phone: Store.s().verifiedPhone || '' }), { showRef: true, narrow: true });
    ...
    return App.buyerShell(TPL.render('a-01', { pkg, seller, draft, pc, g0: pc ? pc.gallery[0] : null }), { showRef: false });
  },
  ...
  A02: function(q) {
    ...
    return App.buyerShell(TPL.render('a-02', { order, pkg, body, holdMin: CONFIG.order.holdSeconds / 60 }), { showRef: true });
  },
  ...
  a02Choose: function(order) {
    ...
    return TPL.render('a-02-choose', { order, sel: this._a02Method || 'gateway', methods });
  },
  ...
  a02Gateway: function(order) {
    ...
    return TPL.render('a-02-gateway', { order });
  },

  /** (c) Khối VietQR + "Tôi đã chuyển khoản" */
  a02Bank: function(order) { return TPL.render('a-02-bank', { order }); },
  ...
  a02Expired: function(order) { return TPL.render('a-02-expired', { order, holdMin: CONFIG.order.holdSeconds / 60 }); },
  ...
  regPrompt: function(order) {
    ...
    UI.modal({ title: 'Trở thành thành viên HOMI365?', sticky: true,
      body: TPL.render('a-03-reg-prompt', { ref, rankLabel: RULES.rank(ref.rank).label, order }),
      foot: TPL.render('a-03-reg-prompt-foot', { order }) });
  },
  /** (a) PAID — mã đơn + mã kích hoạt + mời đăng ký thành viên */
  a03Paid: function(order, pkg) {
    App.after(() => setTimeout(() => this.regPrompt(order), 400));
    return TPL.render('a-03-paid', { order, pkg, info: this.orderInfoBlock(order), cta: this.regCta(order) });
  },
  /** (b) Để sau → xác nhận đã gửi SMS */
  a03Later: function(orderOrId, go) {
    if (go) { App.navigate('A-03?later=1'); return; }
    return TPL.render('a-03-later', { order: orderOrId });
  },
  ...
  a03Failed: function(order) {
    ...
    return TPL.render('a-03-failed', { order, canRetry, info: this.orderInfoBlock(order, { shipping: false }) });
  },
  ...
  a03Awaiting: function(order) {
    ...
    return TPL.render('a-03-awaiting', { order, info: this.orderInfoBlock(order, { shipping: false }) });
  },
  /** (e) Admin từ chối */
  a03Rejected: function(order) { return TPL.render('a-03-rejected', { order, info: this.orderInfoBlock(order, { shipping: false }) }); },
  /** (f) Kho mã hết → chờ cấp */
  a03NoCode: function(order) { return TPL.render('a-03-nocode', { order, info: this.orderInfoBlock(order), cta: this.regCta(order) }); },
  ...
  A04: function(q) {
    ...
    return App.buyerShell(TPL.render('a-04', { intro, mode: this._a04Mode || 'phone', prefill: q.get('phone') || '', demoPhone: Store.user('U101').phone, demoOrderId: paid ? paid.id : '' }));
  },
  a04Phone: function(e) {
    e.preventDefault();
  ...
  a04Result: function(orders, ctx) {
    ...
    body.innerHTML = TPL.render('a-04-result', {
  ...
  A05: function(q) {
    ...
    return App.buyerShell(TPL.render('a-05', { intro, forgot, reason: reason || '', demoPhone: Store.user('U101').phone }));
  },
  a05Submit: function(e) {
    ...
    if (phone === CONFIG.demo.adminPhone) { show('error', 'shield', 'Quản trị viên vui lòng đăng nhập tại trang quản trị', '<a href="#D-00">Tới trang đăng nhập quản trị</a>'); return; }
    ...
    setTimeout(() => {
      ...
      if (r.reason === 'wrong') { show('error', 'x-circle', null, 'Số điện thoại hoặc mật khẩu không đúng. <a href="#A-05?view=forgot">Quên mật khẩu?</a>'); return; }
      // Chưa là thành viên: kiểm tra đơn / hồ sơ đăng ký
      const el = Store.registrationEligibility(phone);
      if (el.reason === 'pending') { show('info', 'clock', 'Hồ sơ thành viên đang ' + UI.label('approval', el.registration.status).toLowerCase(), 'Bạn sẽ nhận email khi hồ sơ được duyệt. <a href="#A-06?state=pending&phone=' + UI.esc(phone) + '">Xem trạng thái</a>'); return; }
      if (el.ok && el.rejected) { show('warning', 'warning', 'Hồ sơ trước đã bị từ chối', UI.esc((el.rejected.approvals.find(a => a.action === 'REJECT') || {}).reason || '') + '. <a href="#A-06?phone=' + UI.esc(phone) + '">Nộp lại hồ sơ</a>'); return; }
      if (el.ok) { show('info', 'user', 'Số này đã mua gói nhưng chưa đăng ký thành viên', '<a href="#A-06?phone=' + UI.esc(phone) + '">Đăng ký thành viên ngay</a> — thông tin được điền sẵn từ đơn hàng.'); return; }
  ...
  a05Forgot: function(e) {
    ...
    UI.otp.mount('fg-otp', { phone, onBack: () => App.reload(), onVerified: () => { body.innerHTML = TPL.render('a-05-reset', { phone }); } });
  },
  ...
  A06: function(q) {
    ...
    return App.buyerShell(TPL.render('a-06', { intro, body }));
  },
  a06Check: function() { return TPL.render('a-06-check', { phoneOk: Store.user('U101').phone, phoneCopper: Store.user('U102').phone }); },
  ...
  a06Blocked: function(ico, title, desc, phone, opt) { const agent = !!(opt && opt.agent); return TPL.render('a-06-blocked', { ico, title, desc, phone, agent, iconClass: agent ? 'is-info' : 'is-pending' }); },
  a06Status: function(reg, phone) {
    ...
    return TPL.render('a-06-status', { reg, phone, steps, rej, referrerName: reg.referrerId ? Store.user(reg.referrerId).fullName : null });
  },
  ...
  a06Done: function(phone) {
    ...
    document.getElementById('a06-body').innerHTML = TPL.render('a-06-done', { regId: r.registration.id, email: p.email, phone, alias: RULES.publicPurchaseUrl(Store.genPurchaseAlias(p.fullName, phone)) });
  }
};
